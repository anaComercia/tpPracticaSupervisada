...
$app->get('/imagen', function(){
    $objeto = new Imagen();
	$data = $objeto->getAll();
	sendResult($data);
});

$app->put('/imagen', function(){
    $request = Slim\Slim::getInstance()->request();
    $data = json_decode($request->getBody(), true); 
	...
	if($result){
		sendResult("Imagenes actualizadas");
	}else{
		sendError("Error al actualizar las imagenes");
	}
});
...
$app->put('/presentacion_producto', function(){
...
$app->delete('/presentacion_producto/:id', function($id){
   
	
	$objeto = new PresentacionProducto();
    $result = $objeto->deleteAll($id);
	
	if($result){
		sendResult("Presentación eliminada");
	}else{
		sendError("Error al eliminar la presentación");
	} 
});
...
$app->delete('/sucursal/:id', function($id){
	...
	if($result){
		...
		sendError("Error al crear al eliminar la sucursal");
	}
});
...
$app->post('/provincia', function(){
    $request = Slim\Slim::getInstance()->request();
    $data = json_decode($request->getBody(), true); //true convierte en array asoc, false en objeto php
	$objeto = new Provincia();
    $result = $objeto->create($data);
	if($result){
		sendResult($result);
	}else{
		sendError("Error al crear la provincia");
	}
});

$app->put('/provincia', function(){
    $request = Slim\Slim::getInstance()->request();
    $data = json_decode($request->getBody(), true); 
	...
	if($result){
		sendResult("Provincia actualizada");
	}else{
		sendError("Error al actualizar la provincia");
	}
});

$app->delete('/provincia/:id', function($id){
    $request = Slim\Slim::getInstance()->request();
    $data = json_decode($request->getBody(), true); //true convierte en array asoc, false en objeto php
	...
	if($result){
		...
		sendError("Error al eliminar la provincia");
	}
});
...
$app->get('/direccion/:id', function($id){
	$localidad = new Localidad();
	$data = $localidad->getDireccionById($id);
	sendResult($data);
});

$app->post('/localidad', function(){
    $request = Slim\Slim::getInstance()->request();
    $data = json_decode($request->getBody(), true); //true convierte en array asoc, false en objeto php
	...
	if($result){
		...
		sendError("Error al crear la localidad");
	}
});

$app->put('/localidad', function(){
    $request = Slim\Slim::getInstance()->request();
    $data = json_decode($request->getBody(), true); 
	...
	if($result){
		sendResult("Localidad actualizada");
	}else{
		sendError("Error al actualizar la localidad");
	}
});

$app->delete('/localidad/:id', function($id){
    $request = Slim\Slim::getInstance()->request();
    $data = json_decode($request->getBody(), true); //true convierte en array asoc, false en objeto php
	...
	if($result){
		...
		sendError("Error al eliminar la localidad");
	}
});
...
$app->get('/tipoDoc', function(){
	$objeto = new Empleado();
	$data = $objeto->getTipoDoc();
	sendResult($data);
});
...
$app->post('/empleados', function(){
	...
	if($result){
		...
		sendError("Error al crear el empleado");
	}
});

$app->put('/empleados', function(){
    $request = Slim\Slim::getInstance()->request();
    $data = json_decode($request->getBody(), true); 
	$objeto = new Empleado();
    $result = $objeto->update($data);
	...
	if($result){
		sendResult("empleado actualizado");
	}else{
		sendError("Error al actualizar al empleado");
	}
});
...
$app->delete('/empleados/:id', function($id){
	...
	$result = $objeto->remove($id);
	if($result){
		sendResult("empleado eliminado");
	}else{
		sendError("Error al eliminar al empleado");
	}
});
...
$app->post('/personas', function(){
	...
	if($result){
		...
		sendError("Error al crear la persona");
	}
});

$app->put('/personas', function(){
    $request = Slim\Slim::getInstance()->request();
    $data = json_decode($request->getBody(), true); 
	...
	if($result){
		sendResult("persona actualizada");
	}else{
...
$app->delete('/personas/:id', function($id){
	...
	$result = $objeto->remove($id);
	if($result){
		sendResult("persona eliminada");
	}else{
		sendError("Error al eliminar la persona");
	}
});
...
$app->put('/usuarios', function(){
	...
	if($result){
		sendResult("usuario actualizado");
	}else{
		sendError("Error al actualizar el usuario");
	}
});

$app->put('/usuariosHabilitacion', function(){
    $request = Slim\Slim::getInstance()->request();
    $data = json_decode($request->getBody(), true); 

	$objeto = new Usuario();
    $result = $objeto->updateHabilitacion($data);
...
$app->delete('/usuarios/:id', function($id){
	...
	$result = $objeto->remove($id);
	if($result){
		sendResult("usuario eliminado");
	}else{
		sendError("Error al eliminar el usuario");
	}
});
...
$app->delete('/envio', function(){
	$objeto = new Envio();
	$data = $objeto->deleteAll();
	if($data){
		sendResult("Envios eliminados");
	}else{
		sendError("Error al eliminar envios");
	}
});

$app->put('/envio', function(){
    $request = Slim\Slim::getInstance()->request();
    $data = json_decode($request->getBody(), true); 
	...
	if($result){
		sendResult("Envios actualizados");
	}else{
		sendError("Error al actualizar envios");
	}
});

$app->post('/inicializarEnvio', function(){
    $request = Slim\Slim::getInstance()->request();
    $data = json_decode($request->getBody(), true); //true convierte en array asoc, false en objeto php
	$objeto = new Envio();
    $result = $objeto->inicializarSucursales($data);
	if($result){
		sendResult($result);
	}else{
		sendError("Error al inicializar los envios");
	}
});
...
$app->post('/inicializarLugar/:id', function($id){
	$objeto = new Lugar();
    $result = $objeto->inicializarLugares($id);
	if($result){
		sendResult($result);
	}else{
		sendError("Error al inicializar los lugares");
	}
});

$app->put('/lugar', function(){
    $request = Slim\Slim::getInstance()->request();
    $data = json_decode($request->getBody(), true); 
	...
	if($result){
		sendResult("Lugares actualizados");
	}else{
		sendError("Error al actualizar lugares");
	}
});
...
//Cupones
$app->get('/cupones', function(){
    $objeto = new Cupon();
	$data = $objeto->getAll();
	sendResult($data);
});

$app->post('/cupones', function(){
    $request = Slim\Slim::getInstance()->request();
    $data = json_decode($request->getBody(), true); //true convierte en array asoc, false en objeto php
	$objeto = new Cupon();
    $result = $objeto->create($data);
	if($result){
		...
		sendError("Error al crear el cupón");
	}
});

$app->put('/cupones', function(){
    $request = Slim\Slim::getInstance()->request();
    $data = json_decode($request->getBody(), true); 
	...
	if($result){
		sendResult("cupón actualizado");
	}else{
		sendError("Error al actualizar el cupón");
	}
});

$app->delete('/cupones/:id', function($id){
	$objeto = new Cupon();
	$result = $objeto->remove($id);
	if($result){
		sendResult("cupón eliminado");
	}else{
		sendError("Error al eliminar el cupón");
	}
});
